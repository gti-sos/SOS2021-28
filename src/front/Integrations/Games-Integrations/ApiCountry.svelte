<script>
  ...
  import { Table, Button, Nav, NavItem, NavLink } from "sveltestrap";
    var APIExt2 = "https://restcountries.eu/rest/v2/lang/es";
  ...
  let ext = [];
    ...
    async function getDataGraph() {
        ...
        */
        ext = await resData.json();
        //console.log(`We have received ${ext.length} resources.`);
        ext.forEach((stat) => {
            countryName.push(parseInt(stat.name));
    ...
    data: {
      x : 'x',
        columns: [
            ...
            ['Populacion en paises aleatorios', countryPopulation[0], countryPopulation[1],countryPopulation[2],countryPopulation[3],countryPopulation[4],countryPopulation[5]],
        ],
        type: 'scatter'
